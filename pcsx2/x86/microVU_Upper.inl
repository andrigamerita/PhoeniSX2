...
static void mVU_FTOIx(mP, const float* addr, microOpcode opEnum)
{
	...
	pass2
	{
		...
		alignas(16) static const uint intmax[4] = __four(0x4F000000); // INT_MAX
		alignas(16) static const uint intmin[4] = __four(0xCF000000); // INT_MIN

		// Note: For help understanding this algorithm see recVUMI_FTOI_Saturate()
		xMOVAPS(t1, Fs);
		if (addr)
			xMUL.PS(Fs, ptr128[addr]);
		xMAX.PS(Fs, ptr128[intmin]);
		xMIN.PS(Fs, ptr128[intmax]);
		xCVTTPS2DQ(Fs, Fs);
