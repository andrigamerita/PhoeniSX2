...
#ifdef __ANDROID__
	EECycleRate = std::clamp<s8>(EECycleRate, -3, 3);
	EECycleSkip = std::min<u8>(EECycleSkip, 3);
#endif
}
...
#else
	wrap.Entry("EmuCore", "AffinityControlMode", AffinityControlMode, AffinityControlMode);
#endif
...
#define GSSettingStringEx(var, name) var = theApp.GetConfigS(name)
	...
	GSSettingBool(OsdShowVersionInfo);
...
#else
	GSSettingIntEnumEx(CRCHack, "CRCHackLevel");
#endif
...
#ifdef __ANDROID__
	UpscaleMultiplier = std::max<u32>(UpscaleMultiplier, 1);
	VsyncQueueSize = std::min(VsyncQueueSize, 3);
