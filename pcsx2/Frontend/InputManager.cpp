...
#ifdef __ANDROID__
		if (si.GetBoolValue("TouchscreenController", "EnableVibration", false) &&
			pad_index == si.GetIntValue("TouchscreenController", "PortIndex", 0))
		{
			PadVibrationBinding binding;
			binding.pad_index = pad_index;

			if (ParseBindingAndGetSource("__DEVICE_VIBRATOR__/Vibrator0", &binding.motors[0].binding, &binding.motors[0].source))
			{
				// combine both motors
				if (vibcaps == PAD::VibrationCapabilities::LargeSmallMotors)
					binding.motors[1] = binding.motors[0];

				s_pad_vibration_array.push_back(binding);
			}
		}
#endif
	}
...
#endif
#ifdef __ANDROID__
#include "Android/AndroidInputSource.h"
#endif
...
#endif
#ifdef __ANDROID__
	UpdateInputSourceState<AndroidInputSource>(si, InputSourceType::Android, true);
#endif
}
