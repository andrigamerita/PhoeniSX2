...
static std::string s_device_info;

void ImGuiManager::SetDeviceInfo(std::string device_info)
{
	s_device_info = std::move(device_info);
}
...
#define DRAW_LINE(font, text, color) \
	...
	if (!paused)
	{
		...
		if (GSConfig.OsdShowVersionInfo)
		{
			text.Clear();
			if (s_device_info.empty())
				text.Write("AetherSX2 " GIT_REV);
			else
				text.Write("AetherSX2 " GIT_REV " (%s)", s_device_info.c_str());
