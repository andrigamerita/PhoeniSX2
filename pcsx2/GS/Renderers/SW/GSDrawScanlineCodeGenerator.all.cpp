...
static void DumpPixFn(u32 x, u32 y, u32 z, u32 w)
{
	...
	if (GSState::s_n < 460)
		return;
	fprintf(fp, "[%u %u] %08X %08X %08X %08X\n", GSState::s_n, counter, x, y, z, w);
	fflush(fp);
}

static void BackupStuff(CodeGenerator& cg)
{
	cg.push(cg.rax);
	cg.push(cg.rbx);
	cg.push(cg.rcx);
...
static void RestoreStuff(CodeGenerator& cg)
{
	cg.add(cg.rsp, 16 * 16);
...
static void BeforePix(CodeGenerator& cg)
{
	BackupStuff(cg);

	cg.mov(cg.rax, reinterpret_cast<uintptr_t>(BeforePixFn));
	cg.call(cg.rax);
