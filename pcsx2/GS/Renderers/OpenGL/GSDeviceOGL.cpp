...
void GSDeviceOGL::RestoreAPIState()
{
	...
	// workaround for mali constant color bug, see OMSetBlendState().
	const float bf = static_cast<float>(GLState::bf) / 128.0f;
	glBlendColor(bf, bf, bf, GLLoader::vendor_id_arm ? 1.0f : bf);
...
void GSDeviceOGL::OMSetBlendState(bool enable, GLenum src_factor, GLenum dst_factor, GLenum op, bool is_constant, u8 constant)
{
	if (enable)
	{
		...
		if (is_constant && GLState::bf != constant)
		{
			...
			// workaround for mali constant color bug, see below
			glBlendColor(bf, bf, bf, GLLoader::vendor_id_arm ? 1.0f : bf);
		}
		...
		if (GLState::f_sRGB != src_factor || GLState::f_dRGB != dst_factor)
		{
			...
			// Mali is an actual piece of garbage, and the destination factor becomes zero when
			// you use a non-constant alpha with a constant color factor... so to work around that,
			// we set the constant alpha to one, and CA/1-CA as factors to mimic 1/0.
			if (is_constant && GLLoader::vendor_id_arm)
				glBlendFuncSeparate(src_factor, dst_factor, GL_CONSTANT_COLOR, GL_ONE_MINUS_CONSTANT_COLOR);
			else
