...
bool GSDeviceVK::CheckFeatures()
{
	...
	const bool isAdreno = (vendorID == 0x5143 || g_vulkan_context->GetDeviceDriverProperties().driverID == VK_DRIVER_ID_QUALCOMM_PROPRIETARY);
	const bool isMali = (vendorID == 0x13B5 || g_vulkan_context->GetDeviceDriverProperties().driverID == VK_DRIVER_ID_ARM_PROPRIETARY);
	...
	m_features.texture_barrier = ((GSConfig.OverrideTextureBarriers == 1) || (GSConfig.OverrideTextureBarriers < 0 && (!isAdreno || m_features.framebuffer_fetch)));
	m_features.broken_point_sampler = isAMD;
	m_features.geometry_shader = features.geometryShader && (GSConfig.OverrideGeometryShaders == 1 || (GSConfig.OverrideGeometryShaders < 0 && !isAdreno && !isMali));
	...
	if (isMali)
	{
		Console.WriteLn("Mali GPU detected, using render pass workaround.");
		m_needs_barrier_between_passes = true;
	}
...
void GSDeviceVK::EndRenderPass()
{
	...
	// Mali seems to crap itself if there's no barrier immediately after a RT gets used, inbetween passes.
	if (m_needs_barrier_between_passes)
	{
		if (m_current_render_target)
		{
			m_current_render_target->GetTexture().TransitionSubresourcesToLayout(g_vulkan_context->GetCurrentCommandBuffer(),
				0, 1, 0, 1, m_current_render_target->GetLayout(), m_current_render_target->GetLayout());
		}
		if (m_current_depth_target)
		{
			m_current_depth_target->GetTexture().TransitionSubresourcesToLayout(g_vulkan_context->GetCurrentCommandBuffer(),
				0, 1, 0, 1, m_current_depth_target->GetLayout(), m_current_depth_target->GetLayout());
		}
	}
}
