...
__fi static void BlockHashResetCRC(BlockHashState& st)
{
	st.acc[0] = 0;
}

__fi static void BlockHashAccumulateCRC(BlockHashState& st, const u8* bp)
{
	const u64* dwords = reinterpret_cast<const u64*>(bp);
	u32 state = static_cast<u32>(st.acc[0]);
	for (u32 i = 0; i < (BLOCK_SIZE / sizeof(u64)); i++)
		state = __crc32d(state, *(dwords++));
	st.acc[0] = static_cast<u64>(state);
}

__fi static void BlockHashAccumulateCRC(BlockHashState& st, const u8* bp, u32 size)
{
	u32 state = static_cast<u32>(st.acc[0]);
...
__fi static GSTextureCache::HashType FinishBlockHashCRC(BlockHashState& st)
{
	return st.acc[0];
}
...
__fi static void BlockHashAccumulate(BlockHashState& st, const u8* bp, u32 size)
{
	if (!GSConfig.LoadTextureReplacements)
	{
		BlockHashAccumulateCRC(st, bp, size);
		return;
	}
