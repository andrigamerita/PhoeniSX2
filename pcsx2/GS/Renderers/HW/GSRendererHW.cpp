...
void GSRendererHW::Draw()
{
	...
	{
		// Ensure draw rect is clamped to framebuffer size.
		const GSVector2i fb_size(rt ? rt->m_texture->GetSize() : ds->m_texture->GetSize());
		m_r = m_r.rintersect(GSVector4i(0, 0, fb_size.x, fb_size.y));
	}
