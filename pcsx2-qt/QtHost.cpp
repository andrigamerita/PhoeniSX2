...
void QtHost::SetDataDirectory()
{
	...
	if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Documents, 0, NULL, &documents_directory)))
	{
		if (std::wcslen(documents_directory) > 0)
			EmuFolders::DataRoot = wxDirName(Path::Combine(wxString(documents_directory), L"AetherSX2"));
		CoTaskMemFree(documents_directory);
	}
#elif defined(__linux__)
	// Check for $HOME/PCSX2 first, for legacy installs.
	const char* home_dir = getenv("HOME");
	const std::string legacy_dir(home_dir ? Path::CombineStdString(home_dir, "aethersx2") : std::string());
	...
	else
	{
		...
		{
			EmuFolders::DataRoot = wxDirName(StringUtil::UTF8StringToWxString(Path::CombineStdString(xdg_config_home, "aethersx2")));
		}
	...
	static constexpr char MAC_DATA_DIR[] = "Library/Application Support/AetherSX2";
	const char* home_dir = getenv("HOME");
	if (home_dir)
...
void QtHost::InitializeDeviceInfo()
{
#ifdef _WIN32
	HKEY hKey;
	if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "HARDWARE\\DESCRIPTION\\System\\BIOS", 0, KEY_READ, &hKey) == ERROR_SUCCESS)
	{
		...
		DWORD gotType;
		DWORD gotSize = sizeof(productName) - 1;
		if (RegQueryValueExA(hKey, "SystemProductName", nullptr, &gotType, reinterpret_cast<LPBYTE>(productName), &gotSize) == ERROR_SUCCESS)
		{
			productName[gotSize] = 0;
			ImGuiManager::SetDeviceInfo(productName);
		}
