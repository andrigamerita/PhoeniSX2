...
void VirtualMemoryReserve::Assign(VirtualMemoryManagerPtr allocator, u8* baseptr, size_t size)
{
	...
	size_t mbytes = size / _1mb;
	if (mbytes)
		mbkb.Write("[%zumb]", mbytes);
	else
		mbkb.Write("[%zukb]", size / 1024);

	DevCon.WriteLn(Color_Gray, "%-32s @ 0x%" PRIXPTR ", -> 0x%" PRIXPTR " %s",
		m_name.c_str(), m_baseptr, m_baseptr + size, mbkb.c_str());
}
