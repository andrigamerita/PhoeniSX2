...
namespace GL
{
	...
	static void DisableBrokenExtensions(const char* gl_vendor, const char* gl_renderer, const char* gl_version)
	{
		if (std::strstr(gl_vendor, "ARM"))
		{
			...
			// Check for newer drivers where this isn't the case.
			int gl_major_version, gl_minor_version, unused_version, major_version, patch_version;
			if (std::sscanf(gl_version, "OpenGL ES %d.%d v%d.r%dp%d", &gl_major_version, &gl_minor_version, &unused_version, &major_version, &patch_version) == 5 &&
				  gl_major_version >= 3 && gl_minor_version >= 2 && major_version >= 32)
			{
				// r32p0 and beyond seem okay.
				Console.Warning("Keeping copy_image for driver version '%s'", gl_version);
			}
			else
			{
				Console.Warning("Mali driver detected, disabling GL_{EXT,OES}_copy_image '%s' '%s' '%s'", gl_vendor, gl_renderer, gl_version);
