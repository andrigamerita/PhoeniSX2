...
void x86capabilities::Identify()
{
	...
	{
		DWORD gotType;
		DWORD gotSize = sizeof(FamilyName) - 1;
		if (RegQueryValueExA(hKey, "ProcessorNameString", nullptr, &gotType, reinterpret_cast<LPBYTE>(FamilyName), &gotSize) == ERROR_SUCCESS)
			FamilyName[gotSize] = 0;
		else
			StringUtil::Strlcpy(FamilyName, "Unknown", sizeof(FamilyName));

		gotSize = sizeof(VendorName) - 1;
		if (RegQueryValueExA(hKey, "VendorIdentifier", nullptr, &gotType, reinterpret_cast<LPBYTE>(VendorName), &gotSize) == ERROR_SUCCESS)
			VendorName[gotSize] = 0;
		else
			StringUtil::Strlcpy(VendorName, "Unknown", sizeof(VendorName));
