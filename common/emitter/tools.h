...
public:
#ifdef _M_X86
	x86VendorType VendorID = x86Vendor_Unknown;
#endif

	uint FamilyID = 0; // Processor Family
	uint Model = 0; // Processor Model
	uint TypeID = 0; // Processor Type
	uint StepID = 0; // Stepping ID

#ifdef _M_X86
	u32 Flags = 0; // Feature Flags
	u32 Flags2 = 0; // More Feature Flags
	u32 EFlags = 0; // Extended Feature Flags
	u32 EFlags2 = 0; // Extended Feature Flags pg2
	u32 SEFlag = 0; // Structured Extended Feature Flags Enumeration
#endif

	char VendorName[32] = {}; // Vendor/Creator ID
